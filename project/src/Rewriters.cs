...
public static class Rewriters
{
    ...
    public static Fst ToRewriter(this Fst fst, Fsa alphabet)
    {
        var all = alphabet.Star();
        var notInDomain = all
    ...
    public static Fst ToLmlRewriter(this Fst fst, ISet<char> _alphabet)
    {
        if (_alphabet.Intersect(markers).Any())
            throw new ArgumentException("The alphabet contains invalid symbols.");

        var allSymbols = FsaBuilder.FromSymbolSet(_alphabet.Concat(markers))
            .Union(FsaBuilder.FromSymbol(Fsa.AnySymbolOutsideAlphabet))
            .Minimal();
        var allSymbolsStarFsa = allSymbols.Star().Minimal();
        var alphabet = allSymbols.Difference(FsaBuilder.FromSymbolSet(markers));
        var alphabetStarFsa = alphabet.Star();
        var cbFsa = FsaBuilder.FromSymbol(cb);
        var lbrbFsa = FsaBuilder.FromSymbolSet(new [] { lb, rb });
        ...
        var initialMatch = // mark the beginnings of all rewrite occurrences by inserting "cb"
            Intro(allSymbols, cbFsa)
                .Compose(
                    LiffR(
                        FsaBuilder.FromSymbol(cb),
                        XIgnore(fstDomain, allSymbols, cbFsa))
                    .Identity());
        ...
        var leftToRight = // insert boundary markers ("lb", "rb") around the leftmost rewrite occurrences
            alphabetStarFsa.Identity() // preceeded by arbitrary text that is not matched by the rule
                .Concat(
                    ...
                    IgnoreX(fstDomain, allSymbols, cbFsa).Identity(), // recognize matches with the leftover "cb" symbol inbetween the markers
        ...
        var includesNotLongestMatches =
            ContainsLang(
                FsaBuilder.FromSymbol(lb)
                    .Concat(
                        IgnoreX(fstDomain, allSymbols, lbrbFsa)
    ...
    static Fst Intro(Fsa alphabet, Fsa symbols) =>
        alphabet.Difference(symbols)
            .Identity()
            .Union(FsaBuilder.FromEpsilon().Product(symbols))
            .Star();

    // Same as "Intro" except symbols from S cannot occur at the end of the string
    static Fst IntroX(Fsa alphabet, Fsa symbols) =>
        Intro(alphabet, symbols)
            .Concat(alphabet.Difference(symbols).Identity())
            .Optional();

    // Same as "Intro" except symbols from S cannot occur at the beginning of the string
    static Fst Xintro(Fsa alphabet, Fsa symbols) =>
        alphabet.Difference(symbols)
            .Identity()
            .Concat(Intro(alphabet, symbols))
            .Optional();

    static Fst XintroX(Fsa alphabet, Fsa symbols)
    {
        var f = alphabet.Difference(symbols).Identity();
