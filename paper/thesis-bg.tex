...
Ще представим формално конструкция на преобразувател под стратегията най-ляво-най-дълго срещане, следвайки алгоритъма на Карттунен (1996) \cite{Karttunen:96}. Целта ни е по зададен краен преобразувател \( T = \langle \Sigma, Q, I, F, \Delta \rangle \), където \( \epsilon \notin dom(T) \) да получим \( R^{LML}(T) \).

Релацията за най-ляво-най-дълго срещане е резултат от композицията на четири релации, които строим независимо една от друга.

\[ R^{LML}(T) := R^{init}(T) \circ R^{left}(T) \circ R^{long}(T) \circ R^{replace}(T) \]

Дефинираме множеството на маркерите, които ще използваме за извършване на заместването - \( \{ cb, lb, rb \} \). Това са произволни символи извън азубиката на входния преобразувател \(T\), \( \Sigma \cap \{ cb, lb, rb \} = \emptyset \). Множеството на всички символи бележим с \( \Sigma_x = \Sigma \cup \{ cb, lb, rb \} \). Дефинираме следните оператори над регулярни езици:

\[ not(L) := \Sigma_x^* \setminus L \]
\[ contain(L) := \Sigma_x^* \cdot L \cdot \Sigma_x^* \]

С \( not(L) \) получаваме допълнението на езика \(L\), \( contain(L) \) ни връща езика, който се състои от всички думи, които съдържат инфикс в \(L\). Следва да дефинираме фамилия от оператори \(intro\) представени от Каплан и Кей (1994) \cite{Kaplan&Kay:94}:
...
\emph{intro(S)} създава релация, която добавя произволно символи от \(S\) в думи, които не съдържат символи от \(S\). На пример, ако имаме азбуката \( \{a,b\} \) и \(S := \{x\}\), то \( \{ \langle b, xb \rangle, \langle ab, axbx \rangle, \langle abba, axxbxbax \rangle \dots \} \in intro(S) \). \emph{introx(S)} и \emph{xintro(S)} са аналогични, като символите от \(S\) не могат да бъдат съответно в края и началото на изходната дума.

Следва да преставим групата от \emph{ignore} оператори \cite{Kaplan&Kay:94}:

\[ ign(L,S) := range(Id(L) \circ intro(S)) \]
\[ ignx(L,S) := range(Id(L) \circ introx(S)) \]
\[ xign(L,S) := range(Id(L) \circ xintro(S)) \]

По даден регулярен език \(L\) и множество от символи \(S\), \emph{ign(L,S)} връща регулярен език, който се състои от думите в \(L\), с включени символи от \(S\). \emph{ignx(L,S), xign(L,S)} са аналогични като не включват символи от \(S\) съответно в края и в началото на думите. Нека разгледаме регулярният език съответстващ на израза \( E := \verb/(a|b)+/ \). Този език съдържа всички думи с дължина поне един символ, които съдържат произволен брой \emph{'a'} и \emph{'b'} - \( \{a, b, ba, ab, abababbb \dots \} \). ign(L(E), \{x\}) ще съдържа думите \emph{ab, xa, xab, xxxb, bbb, bxabx \dots }.
\[ ifSthenP(P,S) = \{ w \mid \forall w_1w_2 = w: w_1 \in P \to w_2 \in S \} \]
\[ ifSthenP(P,S) = \{ w \mid \forall w_1w_2 = w: w_2 \in S \to w_1 \in P \} \]
Чрез тези функции строим филтриращи автомати, с които проверяваме дали маркерите са поставени на коректните места \cite{Kaplan&Kay:94}. \emph{ifPthenS(P,S)} строи автомат, който разпознава всички думи, чиито префикси са в езика на \(P\) и суфикси са в \(S\). Аналогично \emph{ifSthenP(P,S)} разпознава думите, за които ако всичките им суфикси са в \(S\), то всичките префикси са в \(P\).
\[ ifPthenS(P,S) := not(P \cdot not(S)) \]
\[ ifSthenP(P,S) := not(not(P) \cdot S) \]
Комбинираме тези оператори, за да представим:
...
\emph{PiffS(P,S)} разпознава думите, на които префиксите са в \(P\) тогава и само тогава, когато суфиксите им са в \(S\). \emph{LiffR(L,R)} съдържа думите \( w = w_1w_2w_3 \), така че префиксите на \(w_2\) са в \(L\) и суфиксите на \(w_2\) са в \(R\).
\[ PiffS(P,S) := ifPthenS(P,S) \cap ifSthenP(P,S) \]
\[ LiffR(L,R) := PiffS((\Sigma_x^* \cdot L), (R \cdot \Sigma_x^*)) \]
Релацията за задължително заместване \(R^{obl}(T)\) дефинираме както следва:
\[ R^{obl}(T) := N(T) \cdot (T \cdot N(T))^* \]
\[ N(T) = Id(\Sigma^* \setminus (\Sigma^* \cdot dom(T) \cdot \Sigma^*)) \cup \{ \langle \epsilon, \epsilon \rangle \} \]
В случая \(N(T)\) е идентитета на множеството от всички думи, които не съдържат инфикси в \(dom(T)\). Нека разгледаме следният пример:
...
\[ R^{replace}(T) := R_{obl}(\Sigma_x, \{ \langle lb, \epsilon \rangle \} \cdot T \cdot \{ \langle rb, \epsilon \rangle \}) \]

\subsection{Формалнo: Релация за лексически анализ}

\[ G := \{ E_1, E_2 \dots E_n \} \]
\[ 
	T := \bigcup_{i=1}^{|G|} \{ \langle \epsilon, i \cdot \verb/SoT/ \rangle \} \cdot Id(L(E_i)) \cdot \{ \langle \epsilon, \verb/EoT/ \rangle \} 
\]
...
\begin{enumerate}
	...
	\item \emph{Най-дълго-срещане}. Преобразувателят получава изхода от втората съпка и измежду маркираните текстове съдържащи контексти с еднакво начало, извежда такъв, в който фокусите са най-дълги. Тази стъпка отговаря на оператора \(LONGEST\).
	\item \emph{Заместване.} В тази стъпка получаваме изходния текст от стъпка 3, в който поддумите в домейна на преобразувателя са маркирани с \verb/lb/ и \verb/rb/ под стратегията най-ляво-най-дълго срещане. Този преобразувател извършва заместването на тези поддуми чрез входния преобразувател и премахва всички маркери.
\end{enumerate}
...
За да имплементираме тези преобразуватели, нека дефинираме следните оператори над регулярни езици и релации представени в \cite{Kaplan&Kay:94}:
...
static Lexer Create(IList<Rule> grammar)
{
    ...
    var alphabet = unionFst.Transitions
