...
За да построим краен автомат по регулярен израз е нужно първо да дефинираме неговия синтаксис. Въвеждаме следната безконтекстна граматика:
...
Извличането на дърво на извод по зададен регулярен израз реализираме чрез \emph{парсър за рекурсивно спускане (recursive descent parser)}. Той спада към класа парсъри, които строят дървото \emph{от горе надолу (top-down)}, т.е. от корена към листата, започвайки от аксиомата на граматиката (в случая аксиомата е нетерминалът \emph{expr}). Граматическите правила се прилагат от ляво на дясно. В случаите, в които даден нетерминал има повече от един извод е възможно да се наложи да погледнем с един символ напред във входа, за да направим правилния избор. Такъв е случая с нетерминала \emph{expr}. Тъй като граматиката е от тип LL(1), то е достатъчно да погледнем с един символ напред от входа, за да вземем решение кое правило на извод да приложим. Регулярен израз е коректно дефиниран, ако по него може да се построи дърво на извод.
...
\begin{itemize}
	...
	\item Построяването на дървото започва с извикването на метода, отговарящ на аксиомата на граматиката (\emph{Expr()}).
...
Разглеждаме процедурата по създаване на лексическия анализатор, следвайки директно Дефиниция \ref{def:Rlex}
...
\end{minted}
 ...
 Обхождаме правилата в граматиката и по регулярния израз на всяко строим краен автомат (5-7). В последствие по този автомат получаваме преобразувател, който за всяка дума в езика му ще изведе думата с маркери за начало и край (8-11). Избираме символите за край да бъдат извън азбуките на автоматите (8). Обединяваме тези преобразуватели и взимаме азбуката по горната лента на получения преобразувател (15-19). От обединението строим преобразувател под стратегията \emph{най-ляво-най-дълго} срещане, който превръщаме в еквивалентната му бимашина (21-22) и връщаме, създаденият по нея лексически анализатор.
...
Лексическият анализатор съдържа списък от правила (лексическата граматика) и бимашина, построена по съответната граматика. Типът \verb/InputStream/ е интерфейс на входния текст, който може да се подаде като низ от символи в паметта или зареди от текстов файл.
...
Първоначално извличаме редицата от състояния получена от симулацията на десния авомат, прочитайки входния текст от дясно на ляво (16). Ако автоматът не е прочел целия текст, то входът е невалиден и завършваме с грешка (17-19). Започваме симулацията на левия автомат, заедно с извеждането на лексемите. Намираме се в началното му състояние (21), променливата \verb/token/ (22) изпозваме, за да съхраняваме текста на прочетената лексема, \verb/tokenIndex/ (23) индикира коя по ред лексема сме извели, а \verb/tokenStartPos/ (24) показва на коя позиция в текста започва лексемата. Четем входа от ляво на дясно и на всеки прочетен символ формираме тройката от ляво, дясно състояние, и входен символ (28-30). Ако фукнцията на изхода не е дефинирана за тази тройка, хвърляме грешка, в противен случай конкатенираме изходната дума към текста, който сме извели до момента (28-33). Ако последният символ на изведената дума е маркер за край на лемсема (end of token), то сме разпознали лексема и преминаваме към извеждането ѝ (37). В този момент \verb/token/ е низ, който съдържа лексемата и маркерите за начало и край. Ако низът не започва с маркер за начало, то сме попаднали на сегмент в текста, който не е разпознат от нито едно от правилата и съответно хвърляме грешка и извеждаме частта, която не е била разпозната (39-47).

Ако имаме коректно разпозната лексема, то елиминираме маркерите от началото и края така че в \verb/token/ да остане само съдържанието на лексемата (50-51), докато \verb/typeIndex/ (49) представлява идекса на съответното правило в граматиката. На редове (53-59) извеждаме обект, който съдържа данните на разпознатата лексема - нейният индекс, позиция, текст и тип. Чрез ключовата дума \verb/yield/ указваме, че при следващото извикване на \verb/GetNextToken()/, методът ще продължи изпълнението си, от където е приключил. Подготвяме променливите за разпознаването на следващата лексема (61-63) и продължаваме с итерацията над символите на входния текст, докато не стигнем неговия край, или пък докато левия автомат не може да продължи (66-69), което означава, че текстът съдържа невалидна лексема. В края на процедурата правим проверка, че входният текст не завършва с неразпознат сегмент (72-73).
