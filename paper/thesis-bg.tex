...
Ще представим формално конструкция на преобразувател под стратегията най-ляво-най-дълго срещане, следвайки алгоритъма на Карттунен (1996) \cite{Karttunen:96}. Целта ни е по зададен краен преобразувател \( T = \langle \Sigma \times \Sigma^*, Q, I, F, \Delta \rangle \), където \( \epsilon \notin dom(T) \) да получим \( R^{LML}(T) \).

Дефинираме множеството на маркерите, които ще използваме за извършване на заместването - \( \{ cb, lb, rb \} \). Това са произволни символи извън входната азубука на \(T\), \( \Sigma \cap \{ cb, lb, rb \} = \emptyset \). Множеството на всички символи бележим със \( \Sigma_x = \Sigma \cup \{ cb, lb, rb \} \). 
...
\begin{definition}
	Въвеждаме групата от оператори \(intro\) представени от Каплан и Кей (1994) \cite{Kaplan&Kay:94}:
...
\begin{example}
	Нека разгледаме регулярният език съответстващ на израза \( E := \verb/(a|b)+/ \). Този език съдържа всички думи с дължина поне един символ, които съдържат произволен брой \emph{'a'} и \emph{'b'}. \( L(E) = \{a, b, ba, ab, abababbb \dots \} \). \( ign(L(E), \{x\}) \) ще съдържа думите \emph{ab, xa, xab, xabxxb, bbb, bxabx \dots }.	
\end{example}

\begin{definition}\label{def:defIfPThenS}
	Въвеждаме следните функции, които строят филтриращи автомати, чрез които проверяваме дали маркерите са поставени на коректните места \cite{Kaplan&Kay:94}:
	\[ ifPthenS(P,S) = \{ w \mid \forall w_1w_2 = w: w_1 \in P \to w_2 \in S \} \]
	\[ ifSthenP(P,S) = \{ w \mid \forall w_1w_2 = w: w_2 \in S \to w_1 \in P \} \]
	\emph{ifPthenS(P,S)} дефинира езика, който включва всички думи, чиито префикси са в \(P\) и суфикси са в \(S\). Аналогично \emph{ifSthenP(P,S)} представя думите, за които ако всичките им суфикси са в \(S\), то префиксите им са в \(P\). Реализираме тези функции както следва:
...
\begin{definition}\label{def:Robl}
	...
	Идеята е следната: поддумите от входната дума, които са в домейна на релацията \(T\) се превеждат спрямо \(T\). Тези, които не са в този домейн се превеждат чрез идентитет, т.е. входът там остава непроменен.
\end{definition}
...
\begin{definition}\label{def:Rreplace} \emph{Релация на заместване срещане} представяме както следва:
...
\begin{definition} Нека е дадена лексическа граматика
	\[ G := \{ E_1, E_2 \dots E_n \} \]
	където \( E_i, i \in [1, n] \) са регулярни изрази. С \emph{SoT, EoT} са символи, с съответно маркираме началото и края на лексемата във входния текст.  \emph{Релация за лексически анализ} \(R^{lex}\) по зададената граматика \(G\) строим както следва:
